...
./relay examples/e2e_p2p_test.rl -server -port $NODE1_PORT -node-id $NODE1_ID > node1.log 2>&1 &
NODE1_PID=$!
sleep 3

# Start Node 2 and connect to Node 1
echo "🟡 Starting Node 2 on port $NODE2_PORT..."
./relay examples/e2e_p2p_test.rl -server -port $NODE2_PORT -node-id $NODE2_ID -add-peer "http://localhost:$NODE1_PORT" > node2.log 2>&1 &
NODE2_PID=$!
sleep 3

# Start Node 3 and connect to Node 2
echo "🔵 Starting Node 3 on port $NODE3_PORT..."
./relay examples/e2e_p2p_test.rl -server -port $NODE3_PORT -node-id $NODE3_ID -add-peer "http://localhost:$NODE2_PORT" > node3.log 2>&1 &
NODE3_PID=$!
sleep 5
